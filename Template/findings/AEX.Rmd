...
rollg = ugarchroll(gspec, R, n.start = 8100, refit.every = 250, refit.window = 'recursive')
```
...
ESTestingACD <- function(acd.back, var, dist){
  ...
  ES <- roll['Mu'] + roll['Sigma']*apply(roll, 1, function(x)
    integrate(f,0,0.01, skew = x['Skew'], shape = x['Shape'])$value/0.01)
  names(ES) <- "ES"
  ESTest_object <- ESTest(0.01, R[-c(1:8100),], ES[,1], var, conf.level = 0.99, boot = T)
  # length(R)
  # nrow(ES)
  # length(VaR.sstd.egarch)
  AE_ES <- as.numeric(ESTest_object[2])/as.numeric(ESTest_object[1])
...
# DQ Test
DQ_ACD <- GAS::BacktestVaR(R[8101:nrow(R)],rollacd@forecast$VaR$`alpha(1%)`,0.01, 5)$DQ
DQ_nonacd <- GAS::BacktestVaR(R[8101:nrow(R)],rollg@forecast$VaR$`alpha(1%)`,0.01, 5)$DQ
