...
<!-- Steps
1. change title in YAML
2. change data for DAX (in line 38, remember: datasheet 3 or "DAX") 
+ change l44 indices[,XXX] for the column with your data
3. ctrl + f FRANCE CAC 40 and change it 3 times in the footnotes
4. run all
5. check rollacd, if no convergence change n.start to larger number than 2500. Run again
6. l1064: save.image using name DAX.RData, run and comment out
7. l136: add code load("DAX.RData")
8. knit
-->
...
table1 %>% kbl(caption = "Summary statistics of the returns","latex",
  ...
  footnote(general = "Notes",number=c(fn1,fn2,fn3),threeparttable = T,footnote_as_chunk = F, escape=F, general_title = "") %>% landscape()
```
...
# Backtest ----
rollacd = racd::acdroll(spec, R, n.start = 8100, refit.every = 250, refit.window = 'recursive', calculate.VaR = TRUE, VaR.alpha = 0.01)

rollacd1 <- rollacd
for(i in seq(from = 3000, to = 5000, by = 200)){
  tmp.roll <- racd::acdroll(spec, R, n.start = i, refit.every = 250, refit.window = 'recursive', calculate.VaR = TRUE, VaR.alpha = 0.01)
  if(racd::convergence(tmp.roll)==0){
    rollacd1<-tmp.roll
    print(paste0("Convergence FOUND for n.start = ", i))
    break}else{print(paste0("Convergence not found for n.start = ", i))}
}
nstart <- rollacd1@model$n.start
#racd::convergence(rollacd1)
# Benchmark ----
gspec = ugarchspec(mean.model = list(armaOrder = c(1, 0)), variance.model = list(variance.targeting = TRUE), distribution = 'sstd')
rollg = ugarchroll(gspec, R, n.start = nstart, refit.every = 250, refit.window = 'recursive')
```
...
require(dplyr)
# VaRTest----
vartable = rbind(as.data.frame(VaRTest(alpha = 0.01,actual =rollacd1@forecast$VaR[,'realized'],VaR=rollacd1@forecast$VaR[,'alpha(1%)']),row.names =c('ACD(1%)')),
  as.data.frame(VaRTest(alpha=0.01,actual=rollg@forecast$VaR[,'realized'],VaR=rollg@forecast$VaR[,'alpha(1%)']),row.names=c('GARCH(1%)')))

vartable2 <- vartable %>% 
  mutate(AE = actual.exceed/expected.exceed)
...
ESTestingACD <- function(acd.back, var, dist){
  roll <- rollacd1@forecast$density
  f <- function(x, skew, shape) qdist(dist, p = x, mu = 0, sigma = 1, skew = skew, shape = shape)
  #VaR in same fashion as ES below, and gives the same VaR as previously found so it is redundant.
  # VaR <- roll[,'Mu'] + qdist(dist, 0.01, mu = 0, sigma =1, skew = roll[,'Skew'], shape = roll[,'Shape']) * roll[,'Sigma']
  ES <- roll['Mu'] + roll['Sigma']*apply(roll, 1, function(x)
    integrate(f,0,0.01, skew = x['Skew'], shape = x['Shape'])$value/0.01)
  names(ES) <- "ES"
  ESTest_object <- ESTest(0.01, R[-c(1:nstart),], ES[,1], var, conf.level = 0.99, boot = T)
  # length(R)
  # nrow(ES)
  # length(VaR.sstd.egarch)
  AE_ES <- as.numeric(ESTest_object[2])/as.numeric(ESTest_object[1])
...
sacd_sstd_ES <- ESTestingACD(rollacd1, var = as.vector(rollacd1@forecast$VaR$`alpha(1%)`), "sstd")
...
# DQ Test
DQ_ACD <- GAS::BacktestVaR(R[(nstart+1):nrow(R)],rollacd1@forecast$VaR$`alpha(1%)`,0.01, 5)$DQ
DQ_nonacd <- GAS::BacktestVaR(R[(nstart+1):nrow(R)],rollg@forecast$VaR$`alpha(1%)`,0.01, 5)$DQ
...
```{r tableACDBacktest, eco = FALSE, results='asis'}
...
JB.test.df <- suppressWarnings(Table.tests.function(Jarque.bera.test.function, return.matrix = T))
 JB.test.df %>% kbl(caption = "Jarque-Bera Test on standardized residuals",format="latex",
      ...
      digits = 3 )%>%
  kable_classic(full_width = F)%>%
  footnote(general_title = "Notes",general=c(paste0("Table displays the Jarque-Bera statistic $JB=\\\\frac{n}{6}(S^{2}+\\\\frac{1}{4}(K-3)^{2})$ with $n$ the sample size, $K$ the kurtosis and $S$ the skewness for the residuals of the models. The JB statistic is distributed $\\\\chi^2$ with $\\\\nu = 2$. The underlying data is the daily return series of the AEX for the period between ", format(min(index(R)))," and ",format(max(index(R))),"."), "*, ** and *** point out respectively significance at 10, 5 and 1 percent level.", "The null hypothesis is that $S$ and $K$ are not significantly different than what would be found under normality (0 and 3)."),threeparttable = T,footnote_as_chunk = T, escape=F) %>%
  kable_styling(latex_options = "scale_down") %>% landscape()
```
