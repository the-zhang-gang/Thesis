...
rollg = ugarchroll(gspec, R, n.start = 12000, refit.every = 250, refit.window = 'recursive')
...
ESTestingACD <- function(acd.back, var, dist){
  ...
  ES <- roll['Mu'] + roll['Sigma']*apply(roll, 1, function(x)
    integrate(f,0,0.01, skew = x['Skew'], shape = x['Shape'])$value/0.01)
  names(ES) <- "ES"
  ESTest_object <- ESTest(0.01, R[-c(1:12000),], ES[,1], var, conf.level = 0.99, boot = T)
...
# #length var
length(as.vector(rollacd@forecast$VaR$`alpha(1%)`))
length(ES[,1])
...
# DQ Test
DQ_ACD <- BacktestVaR(R[12001:nrow(R)],rollacd@forecast$VaR$`alpha(1%)`,0.01, 5)$DQ
DQ_nonacd <- BacktestVaR(R[12001:nrow(R)],rollg@forecast$VaR$`alpha(1%)`,0.01, 5)$DQ
