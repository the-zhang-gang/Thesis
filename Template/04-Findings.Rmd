...
DistMLE <- function(series) {
  ...
  SGT_start <- list(mu=0,sigma=2, lambda = 0.5, p=2, q=8)
  ...
  SGED_pv_a <- 2* stats::pnorm(-abs(SGED_result$estimate/(SGED_result$std.error)))#to compare with pvalues's of fitdist
...
### SGED (fitdist) #doesnt give the same p values!
#SGED_start_f <- list(mean=0,sd=1, nu = 2, xi = 1.5)
#SGED_result_f <- fitdistrplus::fitdist(data = as.vector(coredata(R)), distr = "sged", method = "mle",       #SGED_start_f)

#SGED_pvalue_b <- 2* stats::pnorm(-abs(SGED_tvalues_f/(SGED_result_f$sd/sqrt(length(R)))))
  ...
  ST_pvalue <- 2* stats::pnorm(-abs(ST_tvalues2))
  ST_significance <- matrix(ncol = 6,nrow = 1)
  
  for(i in c(1:3,6)){
  if(abs(ST_tvalues2[i])<=1.64){
  T_significance[i] <-  "" 
  }
  if(abs(ST_tvalues2[i])>1.64){
  T_significance[i] <-  "*"  
  }
  if(abs(ST_tvalues2[i])> 1.96){
  T_significance[i] <-  "**"  
  }
  if(abs(ST_tvalues2[i])> 2.58){
  T_significance[i] <-  "***"  
  }
  }
  ...
  for(i in c(1:2,6)){
  if(abs(T_tvalues2[i])<=1.64){
  T_significance[i] <-  "" 
  }
  if(abs(T_tvalues2[i])>1.64){
  T_significance[i] <-  "*"  
  }
  if(abs(T_tvalues2[i])> 1.96){
  T_significance[i] <-  "**"  
  }
  if(abs(T_tvalues2[i])> 2.58){
  T_significance[i] <-  "***"  
  }
  }
...
Table2_SE[8,2:4] <- paste0("(",round(ST_result$sd[-3]/sqrt(length(R)),3),")",ST_significance[1:3])
Table2_SE[8,7] <- paste0("(",round(ST_result$sd[3]/sqrt(length(R),3)),")", ST_significance[6])
Table2_SE[10,2:3] <- paste0("(",round(T_result$sd[-3]/sqrt(length(R)),3),")",T_significance[1:2])
Table2_SE[10,7] <- paste0("(",round(T_result$sd[3]/sqrt(length(R),3)),")",T_significance[6])
...
```{r Eurostoxx,results='asis', echo=FALSE}
