...
Estoxx <- indices[-1,1] #see if price index
...
for(i in 1:length(distributions)){
    ...
    list.table.3.tmp[[ref.distr[i]+1]] <- GARCHfit.object[[i]]@fit$robust.tval
}
...
fn2.3 <- sprintf("The mean process is modeled as follows: $R_t= \\\\alpha_0+ \\\\alpha_1 \\\\times R_{t-1}+ \\\\varepsilon_t$ Where, in the %s GARCH models estimated, $\\\\gamma$ is the asymmetry in volatility, $\\\\xi, \\\\kappa$ and $\\\\eta$ are constant and robust $t$ statistics (calculated using robust standard errors based on the method of White (1982)) are displayed in parenthesis. $LLH$ is the maximized log likelihood value.", ncol(Table.3$ST)-1)
```
...
#PVALUES LOOPS
r3 <- c(ES.signific(e_sged_AE_ES_p),ES.signific(gjr_sged_AE_ES_p),ES.signific(t_sged_AE_ES_p),ES.signific(na_sged_AE_ES_p),ES.signific(av_sged_AE_ES_p))
...
r9 <- c(ES.signific(e_ged_AE_ES_p),ES.signific(gjr_ged_AE_ES_p),ES.signific(t_ged_AE_ES_p),ES.signific(na_ged_AE_ES_p),ES.signific(av_ged_AE_ES_p))
...
r15 <- c(ES.signific(e_sstd_AE_ES_p),ES.signific(gjr_sstd_AE_ES_p),ES.signific(t_sstd_AE_ES_p),ES.signific(na_sstd_AE_ES_p),ES.signific(av_sstd_AE_ES_p))
...
r24 <- c(dq.signific(e_std),dq.signific(gjr_std),dq.signific(t_std),dq.signific(v),dq.signific(av_std))
...
```{r lbprep, echo=FALSE, results='asis'}
...
```{r lbtable, echo=FALSE, results='asis'}
...
LJ.test.df.all <- rbind(LJ.test.df, LJ.test.df.2)
LJ.test.df.all <- as.matrix(LJ.test.df.all)
rownames(LJ.test.df.all) <- rep(rownames(LJ.test.df),2)
LJ.test.df.all %>% kbl(caption = "Ljung Box Tests",
      label = 'LjboxTable',
      booktabs = T,
      position = "h!",
      digits = 3 )%>%
  kable_classic(full_width = F)%>%
  pack_rows("Panel A: Ljung Box Test on the standardized absolute values of the residuals", 1,5) %>%
  pack_rows("Panel B: Ljung Box Test on standardized squared residuals", 6,10) %>%
  footnote(general_title = "Notes",general=c(paste0("Table displays the Ljung box statistics for the absolute value of the standardized residuals and for the standardized squared residuals of the models analyzed. The underlying data is the daily return series of the Euro Stoxx 50 for the period between ", format(min(index(R)))," and ",format(max(index(R))),"."), "*, ** and *** point out respectively significance at 10%, 5% and 1% level.", "The null hypothesis of the test in Panel A is described as follows:     $H_0$: $Corr$(|$Z_t$|,|$Z_{t-1}$|)=$Corr$(|$Z_t$|,|$Z_{t-2}$|)= $...$ =$Corr$(|$Z_t$|,|$Z_{t-22}$|) = $0$", "The null hypothesis of the test in Panel B is described as follows:     $H_0$: $Corr$($Z_t^2$,$Z_{t-1}^2$)=$Corr$($Z_t^2$,$Z_{t-2}^2$)= $...$ =$Corr$($Z_t^2$,$Z_{t-22}^2$) = $0$"),threeparttable = T,footnote_as_chunk = F, escape=F) %>%
  kable_styling(latex_options = "scale_down") %>% landscape()
...
#t.test(rugarch::residuals(garchfit.eGARCH[[1]], standardize = T), sd = 1)

# test <- jarqueberaTest(rugarch::residuals(garchfit.eGARCH[[1]], standardize = T))
# lillieTest(rugarch::residuals(garchfit.eGARCH[[1]], standardize = T))
# qqnorm(rugarch::residuals(garchfit.eGARCH[[3]], standardize = T))
# qqline(rugarch::residuals(garchfit.eGARCH[[3]], standardize = T))
# test@test$statistic
# plot(density(rugarch::residuals(garchfit.eGARCH[[3]], standardize = T)))

#LJUNG-BOX TEST ON STANDARDIZED SQUARED RESIDUALS
JB.test <- vector(mode = "list", length = length(Models.garch))
names(JB.test) <- Models.garch
for(i in 1:length(JB.test)){
  JB.test[[i]] <- matrix(NA, nrow = 2, ncol = length(distributions), dimnames = list(c("Statistic", "P Values"), names(Table.3)))
}
#create a function to make Lj box test for squared residuals(((Change the number 22 into how many days back you want to test)))

Jarque.bera.test.function <- function(garchmodel = garchfit.sGARCH, n.list = 1){
  tmp.1 <- lillieTest(rugarch::residuals(garchmodel[[n.list]], standardize = T)) 
  test <- c(tmp.1@test$statistic, tmp.1@test$p.value)
  return(test)
}

for(i in 1:length(distributions)){
  JB.test[[1]][,i] <- suppressWarnings(Jarque.bera.test.function(garchmodel = garchfit.sGARCH, n.list = i))
    JB.test[[2]][,i] <- suppressWarnings(Jarque.bera.test.function(garchmodel = garchfit.eGARCH, n.list = i))
      JB.test[[3]][,i] <- suppressWarnings(Jarque.bera.test.function(garchmodel = garchfit.fGARCH.AVGARCH, n.list = i))
        JB.test[[4]][,i] <- suppressWarnings(Jarque.bera.test.function(garchmodel = garchfit.fGARCH.NAGARCH, n.list = i))
          JB.test[[5]][,i] <- suppressWarnings(Jarque.bera.test.function(garchmodel = garchfit.gjrGARCH, n.list = i))
            JB.test[[6]][,i] <- suppressWarnings(Jarque.bera.test.function(garchmodel = garchfit.fGARCH.TGARCH, n.list = i))
              JB.test[[7]][,i] <- suppressWarnings(Jarque.bera.test.function(garchmodel = garchfit.iGARCH, n.list = i))
                JB.test[[8]][,i] <- suppressWarnings(Jarque.bera.test.function(garchmodel = garchfit.EWMA, n.list = i))
}

JB.test <- lapply(JB.test, round, 3)

for(i in 1:length(JB.test)){
JB.test[[i]][1,which(JB.test[[i]][2,]>=0.10)] <- JB.test[[i]][1,which(JB.test[[i]][2,]>=0.10)]
JB.test[[i]][1,which(JB.test[[i]][2,]<0.10&JB.test[[i]][2,]>0.05)] <- paste0(JB.test[[i]][1,which(JB.test[[i]][2,]<0.10&JB.test[[i]][2,]>0.05)],"*")
JB.test[[i]][1,which(JB.test[[i]][2,]<=0.05&JB.test[[i]][2,]>0.01)] <- paste0(JB.test[[i]][1,which(JB.test[[i]][2,]<=0.05&JB.test[[i]][2,]>0.01)],"**")
JB.test[[i]][1,which(JB.test[[i]][2,]<=0.01)] <- paste0(JB.test[[i]][1,which(JB.test[[i]][2,]<=0.01)],"***")
JB.test[[i]] <- JB.test[[i]][-2,]
}

JB.test.matrix <- cbind(JB.test[[1]],JB.test[[2]],JB.test[[3]],JB.test[[4]],JB.test[[5]],JB.test[[6]],JB.test[[7]],JB.test[[8]])
colnames(JB.test.matrix) <- Models.garch.clean
JB.test.df <- as.data.frame(JB.test.matrix)

JB.test.df %>% kbl(caption = "Jarque-Bera Test on standardized residuals",
      label = 'JBTable',
      booktabs = T,
      position = "h!",
      digits = 3 )%>%
  kable_classic(full_width = F)%>%
  footnote(general_title = "Notes",general=c(paste0("Table displays the Jarque-Bera statistics value for the residuals of the models analyzed. The underlying data is the daily return series of the Euro Stoxx 50 for the period between ", format(min(index(R)))," and ",format(max(index(R))),"."), "*, ** and *** point out respectively significance at 10%, 5% and 1% level.", "The null hypothesis is described as follows: WRITE NULL HYPOTHESIS HERE"),threeparttable = T,footnote_as_chunk = F, escape=F) %>%
  kable_styling(latex_options = "scale_down") %>% landscape()
