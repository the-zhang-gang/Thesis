...
DistMLE <- function(series) {
  ...
  SGED_p <- Inf
  SGED_q <- SGED_result$estimate[3]
  
  plot(xvals, dsged(xvals, mean = SGED_mu, sd = SGED_sigma, xi = SGED_lambda, nu = SGED_q), col="red", type ="l", main = "SGED", ylab="Density")
  lines(density(coredata(series)))
  ...
  GED_lambda <- 0
  GED_p <- Inf
  GED_q <- GED_result$estimate[3]
  plot(xvals, dged(xvals, mean = GED_mu, sd = GED_sigma, nu = GED_q), col="red", type ="l", main = "GED", ylab="Density")
  lines(density(coredata(series)))
  ...
  plot(xvals, dsstd(xvals, mean = ST_mean, sd = ST_sigma, nu = ST_nu, xi=ST_xi), col="red", type ="l", main = "ST", ylab = "Density")
  lines(density(coredata(series)))
  ...
  if(!is.na(Normal_mu)){
  plot(xvals, dnorm(xvals, mean = Normal_mu, sd = Normal_sigma), col="red", type ="l", main = "Normal", ylab="Density")
 lines(density(coredata(series)))}
...
Table2[6,4] <- 2
Table2[6,5] <- Inf
Table2[6,7] <- Normal_result$loglik
...
totalresults <- list(table = Table2_SE)
return(totalresults)
}
...
Eurostoxx <- DistMLE(R)
names(Eurostoxx) <- c("", "$\\mu$","$\\sigma$","$\\lambda$","$p$","$q$","$\\nu$","$L$","AIC")  
```

```{r results='asis'}
kbl(Eurostoxx,caption = "Maximum likelihood estimates of unconditional distribution functions",
      label = 'MLEtable',
      booktabs = T,
      position = "h!",
      digits = 3,
...
for(i in 1:length(distributions)){
    list.table.3.tmp[[ref.distr[i]]] <- garchfit.sGARCH[[i]]@fit$coef
    list.table.3.tmp[[ref.distr[i]+1]] <- garchfit.sGARCH[[i]]@fit$se.coef
}

for(i in 1:length(distributions)){
  list.table.3[[i]] <- t(rbind(list.table.3.tmp[[ref.distr[i]]], list.table.3.tmp[[ref.distr[i]+1]]))
}
...
# merge(list.table.3, by = "col.names", all = T)
# 
# Reduce(function(...) merge(..., by = "col.names", all = TRUE), list.table.3)
# 
