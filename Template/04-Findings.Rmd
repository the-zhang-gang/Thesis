...
```{r MLE tables for different series, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r Euro Stoxx, echo=FALSE, eval=FALSE}
...
# ACD specification
sGARCH_ACDspec = racd::acdspec(mean.model = list(armaOrder = c(1, 0)), variance.model = list(variance.targeting = FALSE),distribution.model = list(model = 'sstd', skewOrder = c(1, 1, 0), shapeOrder = c(1,1,0), skewmodel = 'quad', shapemodel = 'quad'))
...
stopCluster(cl)
cl = makePSOCKcluster(10)
rollacd = racd::acdroll(sGARCH_ACDspec, 
                        data = R[-1,], 
                        n.ahead = 1, 
                        n.start = n2015,
                        refit.every = 25, 
