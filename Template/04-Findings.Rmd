...
table5 <- paste0(object.df[,1]," \\\\\\\\", object.df[,2])
names(table5) <- rownames(object.df)
kbl(table5, col.names = toupper(colnames(object.df)[1]),caption = "Maximum likelihood estimates of the ST-ACD models with constant skewness and kurtosis parameters",format = "latex",
      label = 'Table5',
      booktabs = F, row.names = T, escape = F )%>%
  kable_classic(full_width = F)%>%
  footnote(general = c(fn1.3b,fn2.3),threeparttable = T,footnote_as_chunk = F, escape=F, general_title = "Notes:")%>%
