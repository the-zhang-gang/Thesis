...
DistMLE <- function(R) {
  ...
  ST_significance <- matrix(ncol = 4,nrow = 1)
  for(i in 1:4){
  if(ST_pvalue[i]>=0.10){
  ST_significance[i] <-  "" 
  }
  ...
  T_significance <- matrix(ncol = 3,nrow = 1)
  for(i in 1:3){
  if(T_pvalues[i]>=0.10){
  T_significance[i] <-  "" 
  }
  ...
  Normal_significance <- matrix(ncol = 2,nrow = 1)
  for(i in 1:2){
  if(Normal_pv[i]>=0.10){
...
Table2[2,4] <- Inf
Table2[2,5] <- SGED_result$estimate[3]
...
Table2[3,3] <- 0
Table2[3,4] <- Inf
Table2[3,5] <- GED_result$estimate[3]
...
Table2[4,4] <- 2
Table2[4,5] <- ST_result$estimate[3]
...
Table2[5,3] <- 0 
Table2[5,4] <- 2
Table2[5,5] <- T_result$estimate[3]
...
for(i in 1:length(distributions)){
    list.table.3.tmp[[ref.distr[i]]] <- garchfit.EWMA[[i]]@fit$coef
    list.table.3.tmp[[ref.distr[i]+1]] <- garchfit.EWMA[[i]]@fit$tval
}
...
#cbind(list.table.3.tmp[[ref.distr[1]]], list.table.3.tmp[[ref.distr[1]+1]][match(names(list.table.3.tmp[[ref.distr[1]]]), names(list.table.3.tmp[[ref.distr[1]+1]]))])

# for(i in 1:length(distributions)){
#   #length(list.table.3.tmp[[ref.distr[i]+1]]) <- length(list.table.3.tmp[[ref.distr[i]]])
#   list.table.3[[i]] <- cbind(list.table.3.tmp[[ref.distr[1]]], list.table.3.tmp[[ref.distr[1]+1]])
# }
...
list.restructure <- function(object = list.table.3){
  ...
  for(i in 1:len.table){
      ...
      adj.list[[i]][ref.list[[i]]+1] <- paste0("(", round(list.table.3[[i]][,2],3),")")
  }
  for(i in 1:len.table){
...
table.3.matrix <- cbind(names(adj.list[[3]]), table.3.matrix)
