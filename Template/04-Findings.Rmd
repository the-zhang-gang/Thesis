...
Table.tests.function <- function(function.to.apply = Lj.box.test.function, return.matrix = F){
...
LJ.test.df <- Table.tests.function(Lj.box.test.function, return.matrix = F)
...
```
\noindent In order to check if the models are specified correctly, some specification checks have to be performed. The specification checks have to be done on the standardized residuals of the estimated GARCH model given by the following equation: 
$$ \hat{Z_t} = \dfrac{\hat{\varepsilon_t}}{\hat{\sigma_t}} = \dfrac{R_t - \hat{\mu}}{\hat{\sigma_t}}$$. 

\@ref(tab:LjboxTable) Panel A displays a first Ljung-box test on the absolute value of the standardized residuals of the GARCH models.
...
#WeightedPortTest::Weighted.LM.test(garchfit.eGARCH[[1]]@fit$residuals, garchfit.eGARCH[[1]]@fit$sigma^2, lag = 22) #Li Mak test
...
ARCH.lm.Test.df <- Table.tests.function(archlmtest, return.matrix = F)
...
GMM.test.function <- function(garchmodel=garchfit.eGARCH,n.list = 1){
  z = rugarch::residuals(garchmodel[[n.list]],standardize=TRUE)
...
GMM.test.matrix <- rbind(GMM.test[[1]],GMM.test[[2]],GMM.test[[3]],GMM.test[[4]],GMM.test[[5]],GMM.test[[6]],GMM.test[[7]],GMM.test[[8]])

nrow(GMM.test.matrix)
GMM.test.matrix %>% kbl(caption = "Ljung Box Tests",
      label = 'LjboxTable',
      booktabs = T,
      position = "h!",
      digits = 3 )%>%
  kable_classic(full_width = F)%>%
  pack_rows("Panel A: GARCH", 1,4) %>%
  pack_rows("Panel B: EGARCH", 5,8) %>%
  pack_rows("Panel C: AVGARCH", 9,12) %>%
  pack_rows("Panel D: NAGARCH", 13,16) %>%
  pack_rows("Panel E: GJRGARCH", 17,20) %>%
    pack_rows("Panel F: TGARCH", 21,24) %>%
    pack_rows("Panel G: IGARCH", 25,28) %>%
    pack_rows("Panel H: EWMA", 29,32) %>%
  footnote(general_title = "Notes",general=c(paste0("Table displays the GMM test statistics for the standardized residuals. The underlying data is the daily return series of the Euro Stoxx 50 for the period between ", format(min(index(R)))," and ",format(max(index(R))),"."), "The null hypothesis of the tests for each variable is described as follows:     FORMULA($H_0$ $=$ $...$ HERE!!!"),threeparttable = T,footnote_as_chunk = F, escape=F) %>%
  kable_styling(latex_options = "scale_down") %>% landscape()
```
```{r}
...
JB.test.df <- suppressWarnings(Table.tests.function(Jarque.bera.test.function, return.matrix = F))
...
```{r}
par(mfrow = c(1,2))
qqnorm(rugarch::residuals(garchfit.fGARCH.AVGARCH[[4]], standardize = T), main = "AV GARCH Residuals QQ Plot")
qqline(rugarch::residuals(garchfit.fGARCH.AVGARCH[[4]], standardize = T))
qqnorm(scale(R), main = "Raw standardized returns QQ Plot")
qqline(scale(R))
