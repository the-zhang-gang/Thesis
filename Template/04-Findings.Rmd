...
#create a function to make Lj box test(((Change the number 5 into how many days back you want to test)))

Lj.box.test.function <- function(garchmodel = garchfit.sGARCH, n.list = 1){
  tmp.1 <- Box.test(residuals(garchmodel[[n.list]], standardize = T),5, "Ljung-Box")
...
archlmtest = function (x, lags = 5, demean = FALSE, n.list = 1){
  x <- residuals(x[[n.list]], standardize = T)/sigma(x[[n.list]]) #STANDARDIZE TRUE OR FALSE????????????
