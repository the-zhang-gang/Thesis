...
```{r plotswithing, out.width="100%", fig.cap='Distributions of the data', fig.align='center', eval=FALSE}
...
```{r MLE tables for different series,results='asis', eval=FALSE}
# MLE_Bali <- DistMLE(Rbali)
str(MLE_Eurostoxx)
table2 <- as.data.frame(MLE_Eurostoxx)
table2 <- round(table2,3)
table2[is.na(table2)] <- "" 
table2 %>% kbl(caption = "Unconditional density distributions","latex",
      label = 'disttable',
      booktabs = T,
      position = "h!",
      digits = 3 )%>%
  kable_classic(full_width = F)%>% 
  footnote(general = "Notes",number="Here comes text",threeparttable = T,footnote_as_chunk = F, escape=F, general_title = "") 
```
...
table3[3,1] <- garchfit.sGARCH[[1]]@fit$se.coef[2] #second parameter estimate
