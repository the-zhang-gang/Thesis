...
names.table.3 <- revalue(names(adj.list[[3]]), c("mu"="$\\alpha_0$", "ar1"="$\\alpha_1$", "omega"= "$\\beta_0$", "alpha1"="$\\beta_1$", "beta1"="$\\beta_2$", "gamma1"="$\\gamma$", "skew"="$\\lambda$", "shape"="$\\kappa$", "eta11"="$rot$", "eta21"="$shift$"), warn_missing = F)
...
fn1.3 <- sprintf(paste0('This table shows the maximum likelyhood estimates of various ST-GARCH models. The daily returns used on the ',gsub(pattern = '\\.', replacement=' ',colnames(R))," price index cover the period from ",gsub(" UTC", "",min(index(R)))," to ",gsub(" UTC", "",max(index(R))), "(", nrow(R)," observations)."))
fn2.3 <- paste0("The mean process is modeled as follows: ", "$\\\\\\\\R_t=\\\\alpha_0+\\\\alpha_1 R_{t-1}+\\\\varepsilon_t\\\\\\\\$ ",  "Where, in the ", ncol(Table.3$ST)-1, " GARCH models estimated, $\\\\gamma$ is the asymmetry in volatility, $\\\\lambda$, $\\\\kappa$ and $\\\\eta$ are constant and $t$ statistics are displayed in parenthesis. $LLH$ is the maximized log likelihood value.")

kbl(Table.3$ST,caption = "Maximum likelihood estimates of the ST-GARCH models with constant skewness and kurtosis parameters","latex",
      label = 'dsTable',
      booktabs = T,
      position = "h!",
      digits = 3, row.names = F, escape = F )%>%
  kable_classic(full_width = F)%>% 
  footnote(general = "Notes",number=c(fn1.3, fn2.3),threeparttable = F,footnote_as_chunk = F, escape=F, general_title = "")%>%
