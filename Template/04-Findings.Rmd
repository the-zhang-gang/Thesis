...
colnames(Table2) <- c("alpha","beta","xi","kappa","eta","L","AIC")
...
Table2[4,4] <- 2
Table2[4,5] <- ST_result$estimate[3]/2 # eta parameter
...
Table2[6,1] <- Normal_result$estimate[1]
Table2[6,2] <- Normal_result$estimate[2]
Table2[6,3] <- 0
Table2[6,4] <- 2 
Table2[6,5] <- Inf
...
#fixing rownames
colnames(Table2_SE) <- c("alpha","beta","xi","kappa","nu","L","AIC")
...
Table2_SE[4,2:6] <- paste0("(",round(c(SGED_result$sd[1:2],SGED_result$sd[4],NA,SGED_result$sd[3]), 3),")",SGED_significance)
Table2_SE[6,2:6] <- paste0("(",round(c(GED_result$sd[1:2],NA,NA, GED_result$sd[3]), 3),")",GED_significance)
Table2_SE[8,2:6] <- paste0("(",round(c(ST_result$sd[-3],NA,ST_result$sd[3]),3),")",ST_significance)
...
colnames(table2) <- c("", "$\\mu$","$\\sigma$","$\\lambda$","$p$","$\\nu$","$L$","AIC")  
```
...
for(i in 1:length(distributions)){LLH[i] <- garchfit.iGARCH[[i]]@fit$LLH}
names.table.3 <- revalue(names(adj.list[[3]]), c("mu"="$\\alpha_0$", "ar1"="$\\alpha_1$", "omega"= "$\\beta_0$", "alpha1"="$\\beta_1$", "beta1"="$\\beta_2$", "gamma1"="$\\gamma$", "skew"="$\\lambda$", "shape"="$\\kappa$"), warn_missing = F)
...
table.3.matrix <- cbind(c(names.table.3,"$\\eta$","","LLH"), table.3.matrix)
...
#Testing the kabling
knitr::kable(Table.3.eGARCH)

# Table.3.cbind.function <- function(distribution = "sstd"){}
# 
# Table.3.fGARCH.AVGARCH[Table.3.fGARCH.AVGARCH[,1]!="",1] 
# 
# 
# test1 <- Table.3.iGARCH[c(match(Table.3.fGARCH.AVGARCH[Table.3.fGARCH.AVGARCH[,1]!="",1],Table.3.iGARCH[,1]),match(Table.3.fGARCH.AVGARCH[Table.3.fGARCH.AVGARCH[-nrow(Table.3.fGARCH.AVGARCH),1]!="",1],Table.3.iGARCH[,1])+1),-1]
# 
# hello1 <- Table.3.iGARCH[,4:5]
# hello2 <- Table.3.fGARCH.AVGARCH[,4:5]
# 
# 
# fff <- cbind(Table.3.fGARCH.AVGARCH[,1],Table.3.iGARCH[match(Table.3.fGARCH.AVGARCH[,1],Table.3.iGARCH[,1]),-1])
# 
# 
# test <- c(match(Table.3.fGARCH.AVGARCH[Table.3.fGARCH.AVGARCH[,1]!="",1],Table.3.iGARCH[,1]),match(Table.3.fGARCH.AVGARCH[Table.3.fGARCH.AVGARCH[-nrow(Table.3.fGARCH.AVGARCH),1]!="",1],Table.3.iGARCH[,1])+1)
#                                                                                                    
# test1[is.na(test1)]
#                                                                                                  
# test[!is.na(test)] <- sort(test[!is.na(test)])
# 
# 
# Table.3.iGARCH[,which(names(Table.3.iGARCH)=="sstd")]
# 
# hi <- left_join(hello1, hello2, na_matches = "na")
