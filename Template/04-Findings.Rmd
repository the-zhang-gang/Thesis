...
LJ.test.df %>% kbl(caption = "Ljung Box Test",
      ...
      digits = 3 )%>%
  kable_classic(full_width = F)%>%
  footnote(general = "Notes",number=c(paste0("Table displays the Ljung box statistics value for the models analyzed. The underlying data is the daily return series of the Euro Stoxx 50 for the period between ", format(min(index(R)))," and ",format(max(index(R))),"."), "*, ** and *** point out respectively significance at 10%, 5% and 1% level."),threeparttable = T,footnote_as_chunk = F, escape=F, general_title = "") %>%
  kable_styling(latex_options = "scale_down") %>% landscape()
```
