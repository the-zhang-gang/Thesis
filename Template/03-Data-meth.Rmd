...
require(ftplottools)
theme_set(ft_theme() +theme(legend.position = "none"))

options(scipen = 999)
knitr::opts_chunk$set(message = FALSE)
knitr::opts_chunk$set(warning = FALSE)
data <- read_excel("data/datastream.xlsx",col_types = c("date", rep("numeric", 6)),skip = 2) 
...
$s_{i t}$ = Number of shares of company ($i$) at time (t) 
$f f_{i t}$ = Free float factor of company ($i$) at time (t) 
$c f_{i t}$ = Weighting cap factor of company ($i$) at time (t) 
$x_{i t}$ = Exchange rate from local currency into index currency for company ($i$) at time (t) 
...
\@ref:stats provides the main statistics describing the return series analyzed. Returns are computed with the \@ref(eq:log.returns) formula
...
\noindent The Arithmetic Mean of the series is `r round(as.numeric(table1[1,2]),3)`% with a standard deviation of `r round(as.numeric(table1[5,2]),3)`% and a median of `r round(as.numeric(table1[2,2]),3)` which translates to an annualized mean of `r round(as.numeric(table1[1,2])*252,3)`% and an annualized standard deviation of `r round(as.numeric(table1[5,2])*sqrt(252),3)`%. The Skewness statistic is `r round(as.numeric(table1[6,2]),3)` with a significance level of `r round(as.numeric(table1[7,2]),3)*100`% and the excess Kurtosis is `r round(as.numeric(table1[8,2]),3)` significant at the `r round(as.numeric(table1[9,2]),3)*100`% level giving an overview of the distribution of the returns which has thicker tails than the normal distribution with a higher presence of left tail observations. A formal test such as the Jarque-Bera one with its statistic at `r round(normtest::jb.norm.test(R)$statistic,3)` significant at the `r round(normtest::jb.norm.test(R)$p.value *100,3)`% level confirms the non normality feeling given by the Skewness and Kurtosis.

\noindent The right column of \@ref:stats displays the same descriptives but for the standardizes residuals obtained from a simple GARCH model as mentioned in \@ref:stats in Note 2$*$. Again Skewness statistic at `r round(as.numeric(table1[6,3]),3)` with a significance level of `r round(as.numeric(table1[7,3]),3)*100`% and the excess Kurtosis at `r round(as.numeric(table1[8,3]),3)` significant at the `r round(as.numeric(table1[9,3]),3)*100`% level
...
## RETURNS
round(as.numeric(table1[1,2]),3)
round(normtest::jb.norm.test(R)$statistic,3)
...
fn1 <- sprintf(paste0('Note 1: This table shows the descriptive statistics of the daily percentage returns of %s over the period %s to %s (', nrow(R),' observations). Including minimum, median, arithmetic average, maximum, standard deviation, skewness, excess kurtosis and the Jarque-Bera test. \n'),gsub(pattern = '.', replacement=' ',colnames(R)),gsub(" UTC", "",min(index(R))),gsub(" UTC", "",max(index(R))))
fn2 <- 'Note 2*: The standardized residual is derived from a maximum likelyhood estimation (simple GARCH model) as follows: \n' 
fn3 <- '$R_t=\\alpha_0+\\alpha_1 R_{t-1}+z_t \\sigma_t$ \n'
fn4 <-  '$\\sigma_t^2=\\beta_0+\\beta_1 \\sigma_{t-1}^2 z_{t-1}^2+\\beta_2 \\sigma_{t-1}^2,$ \n Where $z$ is the standard residual (assumed to have a normal distribution) \n'
fn5 <- 'Note 3: *, **, *** represent significance levels at the 5%, 1% and <1%'
...
table1 %>% kbl(caption = "Summary statistics of the returns",
  ...
  footnote(c(fn1,fn2,fn3,fn4, fn5),threeparttable = T,
...
##### Stylized facts
As can be seen in figure \@ref(fig:plot1) the Euro area equity and later, since 1999 the EuroStoxx 50 went up during the tech ("dot com") bubble reaching an ATH of €`r max(Estoxx)`. Then, there was a correction to boom again until the burst of the 2008 financial crisis. After which it decreased significantly. With an ATL at `r format(index(Estoxx)[Estoxx == min(Estoxx["2000/"])], '%d %B, %Y')` of €`r min(Estoxx["2000/"])`. There is an improvement, but then the European debt crisis of 2011 occurred. With then some improvement again, with the decline beginning of 2020. To then recover very quickly to already values higher then the pre-COVID crisis level.

```{r plot1, echo=F, fig.cap='Eurostoxx 50 Price Index prices', fig.align='center', out.width="75%"}
par(mfrow = c(1,1))
...
# ggplot(Estoxxdf) + geom_line(aes(x=Date, y = `EURO STOXX 50`), color = "steelblue") + ft_theme() + geom_vline(xintercept = events, linetype="dotted", size = 0.3) + geom_text(aes(x = as.Date("1997-01-01"), label = "Launch of Euro", y =1300), colour = "black", angle = 90) + geom_text(aes(x = as.Date("1999-01-01"), label = "Dot com bubble", y =1400), colour = "black", angle = 90) + geom_text(aes(x = as.Date("2007-01-01"), label = "Financial crisis", y =1400), colour = "black", angle = 90)+ geom_text(aes(x = as.Date("2014-01-01"), label = "Eurobond crisis", y =1400), colour = "black", angle = 90) + geom_text(aes(x = as.Date("2020-01-01"), label = "COVID-19 crisis", y =1400), colour = "black", angle = 90) + ylab("Price")

plot(as.zoo(Estoxx), screen = 1, col = "steelblue", xlab = "Date", ylab = "Eurostoxx 50 Price", main = "Price"); grid(); abline(v= index(Estoxx$`EURO STOXX 50`[1000]))


```

\newpage
In figure \@ref(fig:plot2) the daily log-returns are visualized. There is already 
```{r plot2, echo=F, fig.cap='Eurostoxx 50 Price Index log returns', fig.align='center', out.width="75%"}
Rdf <- data.frame(Date = as.Date(index(R)), `Log Returns` = R)
ggplot(Rdf) + geom_line(aes(x=Date, y = EURO.STOXX.50), color = "steelblue") + ft_theme() +ylab("Log returns")
# plot(as.zoo(R), screen = 1, col = "steelblue", xlab = "Date", ylab = "Eurostoxx 50 Price Log Returns",main = "Log Returns");grid()
```

```{r plot3, echo=F, fig.cap='Eurostoxx 50 rolling volatility (22 days, calculated over 252 days)', out.width= "75%", fig.align='center'}
#Plotting volatility
par(mfrow = c(1,1))
realized.vol <- xts(apply(R,2,runSD,n=22), index(R))*sqrt(252)
r.voldf <- data.frame(Date = as.Date(index(realized.vol)), `Volatility` = R)
ggplot(Rdf) + geom_line(aes(x=Date, y = EURO.STOXX.50), color = "steelblue") + ft_theme() +ylab("Log returns")
...
acf(abs(e), main = "ACF (absolute prediction errors)", xaxt = "n")
plot(e^2,main = "Squared Prediction Error")
acf(e^2, main = "ACF (squared prediction errors)", xaxt = "n")
...
(\#eq:densityconddens)
```
Again @ghalanos2015 makes it easier to implement the somewhat complex ACD models using the R language with package "racd".
...
@engle1999 with the aim to provide completeness to the conditional coverage test of @christoffersen2001 developed the Dynamic quantile test. It consists in testing some restriction in a
