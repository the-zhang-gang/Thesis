...
# Estimate the model
garchfit_R <- ugarchfit(data = R, spec = garchspec_R)

# Compute stdret using residuals()
stdret_R <- residuals(garchfit_R, standardize = TRUE)

Statistics.S <-  table.Stats(stdret_R)
...
# Skewness test
skewtest <- normtest::skewness.norm.test(coredata(stdret_R)) #as you can see the skewness is the same as the table.Stats method
Skewness <- round(skewtest$statistic,4)
names(Skewness) <- "Skewness"
skewness.pvalue <- skewtest$p.value
kurttest <- normtest::kurtosis.norm.test(coredata(stdret_R)) #this is normal kurtosis (not excess)
Excess_kurtosis <- round(kurttest$statistic,4) - 3
names(Excess_kurtosis) <- "Excess Kurtosis"
kurtosis.pvalue <- kurttest$p.value

# Jb test
robustjb_R <- DescTools::JarqueBeraTest(coredata(stdret_R)) #robust?
jb_R <- normtest::jb.norm.test(stdret_R)
...
```{r plot1, echo=F, fig.cap='Eurostoxx 50 prices and returns', out.width= "50%"}
par(mfrow = c(2,1))
plot(Estoxx, major.ticks = "years", grid.ticks.on = "years", col = "steelblue",
     main = "EuroStoxx 50 Price Index")
plot(R, major.ticks = "years", grid.ticks.on = "years", col = "steelblue",
     main = "EuroStoxx 50 Price Log Returns")
```

```{r plot2, echo=F, fig.cap='Eurostoxx 50 rolling volatility (22 days, calculated over 252 days)', out.width= "50%"}
#Plotting volatility
par(mfrow = c(1,1))
chart.RollingPerformance(R = R, width = 22,FUN = "sd.annualized", scale = 252, main = "One month rolling volatility", colorset="steelblue")
```

```{r plot3, echo=F,fig.cap='Density vs. Normal Eurostoxx 50 log returns)', out.width= "50%"}
...
```{r acfplots, echo=F, fig.align='center', fig.cap="Absolute prediction errors", fig.subcap="This figure shows the absolute prediction errors and the autocorrelation function for the Eurostoxx 50.", out.width= "50%"}
...
plot(abs(e), main = "Absolute Prediction Error", col = "steelblue")
