...
R <- diff(Estoxx, log = TRUE, na.pad = FALSE) 
...
kable(Statistics, caption = "Summary statistics of the returns",
      label = 'dsTable',
      booktabs = T,
      position = "h!",
      digits = 3 )%>%
...
```{r}
distributions <- c("norm", "snorm", "std", "sstd", "sged")
...
garchspec[[i]] <- ugarchspec(mean.model = list(armaOrder = c(0,0)),
...
#Remove the chart histogram from the loop

chart.Histogram(stdret[[i]], methods = c("add.normal","add.density" ), 
