...
### Alternative distributions than the normal

#### Student t distribution

One, often used alternative for the normal distribution is the Student t distribution. It is also a symmetric distribution, this means skewness is equal to zero. The probability density function (pdf), again following @annaert2021, is given by equation \@ref(eq:std). As will be seen in \@ref(vol-mod), GARCH models are used for volatility modeling in practice. @bollerslev1987 examined the use of the GARCH-Student model as an alternative to the standard Normal distribution.
...
```{=tex}
\begin{align}
f(x) = \dfrac{\Gamma(\dfrac{\nu+1}{2}}{\Gamma(\dfrac{\nu}{2})\sqrt{\beta \pi \nu}} (1+\dfrac{(x-\alpha)^2}{\beta \nu})^{-(\nu+1)/2}
 (\#eq:stdghalanos)
\end{align}
```
where $\alpha, \beta$ and $\nu$ are the location parameters (scale and shape parameters). The symbol $\Gamma$ is the Gamma function.
...
The GED distribution is used in the GED-GARCH model by @nelson1991. This model replaced the assumption of conditional normally distributed error terms by
...
The SGT distribution, introduced by @theodossiou1998 and applied by @bali2007 and @bali2008. The generalized-T distribution was introduced by
...
The eGARCH model or exponential GARCH model [@nelson1991] is defined as in equation \@ref(eq:eq16),
...
#### avGARCH model (in our paper: TS-GARCH to Taylor and Schwert)
