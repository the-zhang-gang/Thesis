...
As can be seen in the following image \@ref(fig:sgtpackagefigure), the SGT distribution is visualized from @davis2015 (who composed this based on @hansen2010). As you can see, the SGT has many special cases. As already mentioned (the SGED is for example the SGT specification with parameter q, the second kurtosis parameter of the SGT, equal to infinity).

```{r sgtpackagefigure, fig.align='center', out.width='90%', fig.cap = 'Distributions, from Hansen, McDonald and Newey (2010), following Carter Davis (2015)'}
knitr::include_graphics('figures/SGTdistributions.png')
```
...
All the GARCH models below are estimated with the R language using the package "rugarch" by @alexios2020. We use specifications similar to @ghalanos2020. Parameters have to be restricted so that the variance output always is positive, except for the EGARCH model, as this model does not mathematically allow for a negative output. An overview (of a selection) of GARCH models is given in the following table.
...
```{=tex}
\begin{align}
{P} = \sum\limits_{j = 1}^q {{\alpha_j}}  + \sum\limits_{j = 1}^p {{\beta_j}}.
 (\#eq:eq14)
\end{align}
```
...
\noindent The TGARCH or threshold model [@zakoian1994] also models assymetries in volatility depending on the sign of the shock, but contrary to the GJRGARCH model it uses the conditional standard deviation instead of conditional variance. It is specified as in equation \@ref(eq:eq19).
...
#### AVGARCH/TSGARCH model

\noindent The Absolute Value GARCH model (AVGARCH) or TSGARCH model, as named after @taylor1986 and @schwert1989, models the conditional standard deviation and is intuitively specified like a normal GARCH model, but with the absolute value of the shock term. It is specified as in equation \@ref(eq:eq20).
...
\noindent A alternative to the series of GARCH models is the exponentially weighted moving average or EWMA model. This model calculates conditional variance based on the shocks from previous periods. The idea is that by including a smoothing parameter $\lambda$ more weight is assigned to recent periods than distant periods. The $\lambda$ must be less than 1. It is specified as in equation \@ref(eq:eq21).
...
<!--#  think we need 2 pages of this, while this is the most important part -->
