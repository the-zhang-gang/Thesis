...
\sethlcolor{pink}\hl{\textbar Note for prof. Annaert: choices: n.start = 1500 days before the end of the series, refit.every = 252 (trading days in a year), solver = hybrid using a cluster = 10 to run on 10 cores to speed up the process of estimation of the roll object (took 5-10 minutes per backtest with some solvers, now with parallel package\ldots)\textbar{}}\sethlcolor{correctioncolor}
...
{\centering \includegraphics{_main_files/figure-latex/figurebacktests2-1} 
