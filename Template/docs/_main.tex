...
\begin{acknowledgements}
 	First of all, we want to acknowledge the support during the writing of this thesis, given to us by our families and loved ones. Secondly, we are gratefull to our professors Jan Annaert\footnote{\url{https://www.antwerpmanagementschool.be/nl/faculty/jan-annaert}}, Marc De Ceuster\footnote{\url{https://www.antwerpmanagementschool.be/nl/faculty/marc-de-ceuster}} and Hairui Zhang\footnote{\url{https://www.antwerpmanagementschool.be/nl/faculty/hairui-zhang}} for all the valuable insights during our courses at AMS and dozens of assignments using the R language. We are especially gratefull that they patiently answered all our questions throughout the year.~\\

  \noindent Secondly, we want to extend our appreciation to \href{https://www.cs.ox.ac.uk/people/ulrik.lyngs/}{Ulrik Lyngs} who made it easier to work together in R with a pre-build template for the university of Oxford, without which this thesis could not have been written in this format \autocite{lyngsOxforddown2019}. ~\\

  \noindent Finally, we thank \href{https://www.linkedin.com/in/alexios-galanos-64309165/}{Alexios Ghalanos} for making the implementation of GARCH models integrated in R via his package ``rugarch''. By doing this, he facilitated the quantitative analysis we did in our methodology. ~\\
...
\caption{\label{tab:Table3}Maximum likelihood estimates of the GARCH-ST models with constant skewness and kurtosis parameters}
\centering
\begin{threeparttable}
\resizebox{\linewidth}{!}{
...
\caption{\label{tab:Table3b}Maximum likelihood estimates of the GARCH-SGED models with constant skewness and kurtosis parameters}
\centering
\begin{threeparttable}
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{lllllllll}
...
\noindent Table \ref{tab:LjboxTable} contains the results of two tests. Panel A displays the Ljung-box test on the squared standardized residuals of the GARCH models. Panel B displays the ARCH LM test on the squared standardized residuals. The ARCH LM test checks if the ARCH process is adequately fitted. As can be seen for both tests, the EGARCH, AVGARCH, NAGARCH and TGARCH do not suffer from any correlation in the squared residuals. For EWMA there is highly significant correlation of the residuals for all underlying distributions, suggesting that the model is inappropriate for VaR and CVaR estimation.
...
\noindent The Generalized Method of Moments (GMM) test by \textcite{hansen1982} is also a test that checks if the model is correctly specified or not, by examining the moments of the standardized residuals. Here only the results of the t-tests for the four individual moments are examined from the GMM test{[}\^{}findings-3{]}. The mean (first moment) should be equal to zero (\(E\left[z_{t}\right]=0\)). The variance (second moment) should be equal to one (\(E\left[z_{t}^2-1\right]=0\)). The skewness should be equal to zero and the excess kurtosis (third and fourth moment) should be equal to zero (respectively \(E\left[z_{t}^3\right]=0\) and \(E\left[z_{t}^4-3\right] = 0\)). Table \ref{tab:GMMtable} portrays the results of the GMM test. Only the AVGARCH and NAGARCH models fail to reject the null-hypothesis that the moments are significantly different from the normal distribution for all underlying distributions. TGARCH fails to reject the null-hypothesis for all distributions, except for the Student-t. Similarly, EGARCH fails to reject it for the GED and Student-t. The SGARCH, IGARCH and EWMA reject the null-hypothesis with high significance for all distributions and are thus not suited for VaR and CVaR estimation.
...
Figure \ref{fig:figuresqq} shows again that the normal distribution is not a good choice as an assumption for the innovation process. The best models seem to be the ST and the SGED. While this is just eye-balling, it should be noted that our results point in the same direction: skewed distributions seem to model the data better.

\begin{figure}[h]

{\centering \includegraphics[width=1\linewidth]{figures/qqplots} 

}

\caption{QQ plots of AVGARCH residuals using various distributions}\label{fig:figuresqq}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{_main_files/figure-latex/mleplots-1.pdf}
\caption{\label{fig:mleplots}Theoretical (red) vs.~Actual (black) distributions of Euro Stoxx 50}
\end{figure}
...
\noindent Second, in figure \ref{fig:aic2}, symmetric GARCH models with the best symmetric distribution (T) and other distributions (SGED, ST) are looked at. As you can see the SGED seems to perform better than the skewed student's t distribution (ST) and the latter is better than the scaled T distribution. This is consistent with \textcite{giot2003} that the skewed student's t (ST) distribution outperforms the symmetric distributions.
