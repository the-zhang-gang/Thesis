...
\noindent Autoregressive Conditional Heteroscedasticity (ARCH) models, proposed by @engle1982, was in the first case not used in financial markets but on inflation. Since then, it has been used as one of the workhorses of volatility modeling.
...
\noindent The arithmetic mean of the series is `r round(as.numeric(levels(table1[1,2][,drop=T])),2)`% with a standard deviation of `r round(as.numeric(levels(table1[5,2][,drop=T])),2)`% and a median of `r round(as.numeric(levels(table1[2,2][,drop=T])),2)` which translate to an annualized mean of `r round(as.numeric(levels(table1[1,2][,drop=T]))*252,2)`% and an annualized standard deviation of `r round(as.numeric(levels(table1[5,2][,drop=T]))*sqrt(252),2)`%. The skewness statistic is highly significant and negative at `r round(as.numeric(levels(table1[6,2][,drop=T])),2)` and the excess kurtosis is also highly significant and positive at `r round(as.numeric(levels(table1[8,2][,drop=T])),2)`. These 2 statistics give an overview of the distribution of the returns which has thicker tails than the normal distribution with a higher presence of left tail observations. A formal test such as the Jarque-Bera one with its statistic at `r round(normtest::jb.norm.test(stdret.R)$statistic,2)` and a high statistical significance, confirms the non normality feeling given by the Skewness and Kurtosis.
...
```{r plot1, echo=F, fig.cap='Euro Stoxx 50 Price Index prices', fig.align='center', out.width="95%", fig.pos='h'}
par(mfrow=c(1,1))
...
```{r plot2, echo=F, fig.cap='Euro Stoxx 50 Price Index log returns', fig.align='center', out.width="70%", fig.pos='h'}
...
# dev.off()
knitr::include_graphics('figures/vol-clustering-finalized-2.png')
```
...
```{r plot3, echo=F, fig.cap='Euro Stoxx 50 rolling volatility (22 days, calculated over 252 days)', out.width= "65%", fig.align='center', fig.pos='h'}
#Plotting volatility
par(mfrow = c(1,1))
...
```{r plot4, echo=F,fig.cap='Density vs. Normal Euro Stoxx 50 log returns)', out.width= "75%", fig.align='center', fig.pos='h'}
...
After estimation of the GARCH models in-sample, out-sample analysis is done by performing a rolling window approach. With assumptions: a window of 2500 observations and re-estimation every year.

[\|**Note for profs: choices**:\
...
\noindent Following @ghalanos2016, arguments of ACD models are specified as in @hansen1994. The density function $f(y|\alpha)$ has parameters $\alpha_t = (\mu_t, \sigma_t, \eta_t)$, with equation \@ref(eq:cmean), the conditional mean equation. Equation \@ref(eq:cvariance) as the conditional variance. And $\eta_t=\eta(\theta,x_t)$ the remaining parameters of the distribution like the skewness and kurtosis (shape) parameters which are constant in GARCH models (or time-invariant).
...
The Expected Shortfall test by @mcneil2000 tests whether the excess conditional shortfall has zero mean. Under the alternative hypothesis, this mean is greater than zero. This test uses a one sided t-statistic and bootstrapped p-values, as the distribution of the excess conditional shortfall is not assumed to be normal.
...
data <- suppressWarnings(read_excel("data/datastream.xlsx",col_types = c("date", rep("numeric", 6)),skip = 2)) #warnings are NA's
colnames(data) <- c("Date",gsub(pattern = " - PRICE INDEX", replacement='' , colnames(data)[2:7]))
Price_indices <- as.xts(data[,-1], order.by = data$Date)
Estoxx <- Price_indices[-1,1] #see if price index
R <- diff(Estoxx, log = TRUE, na.pad = FALSE)*100
...
```{r mlesgt, echo=FALSE, include=FALSE}
require(sgt)
require(graphics)
require(stats)
...
```{r table2prep, include=FALSE, echo=FALSE}
...
kbl(table2,col.names= c("$\\theta$", "$\\alpha$","$\\beta$","$\\xi$","$\\kappa$","$\\eta$","$LLH$","AIC") ,caption = "Maximum likelihood estimates of unconditional distribution functions",
  ...
  kable_classic(full_width = F)%>%
  footnote(general = "Table contains parameter estimates for SGT-distribution and some of its limiting cases.  \\\\\\\\ The underlying data is the daily return series of the Euro Stoxx 50for the period between December 31. 1986 and April 27. 2021. Standard errors are reported between brackets. $LLH$ is the maximum log-likelihood value. *, ** and *** point out significance at 10%, 5% and 1% level.",threeparttable = T,footnote_as_chunk = F, escape=F, general_title = "Notes") %>%
...
fn1.3 <- sprintf(paste0('This table shows the maximum likelihood estimates of various ST-GARCH \\\\\\\\ models. The daily returns used on the ',stringr::str_to_title(gsub(pattern = '\\.', replacement=' ',colnames(R)))," Price index cover the  \\\\\\\\ periodfrom ",gsub(" UTC", "",format(min(index(R)), '%d %B, %Y'))," to ",gsub(" UTC", "",format(max(index(R)),'%d %B, %Y')), " (", nrow(R)," observations)."))
...
aics %>% kbl(caption = "Model selection according to AIC",
      ...
      digits = 3 )%>%
  kable_classic(full_width = F)%>%
  footnote(general_title= "Notes",general="This table shows the AIC value for the respective model",threeparttable = T,footnote_as_chunk = F, escape=F) %>%
  kable_styling(latex_options = "scale_down") %>% landscape()
```
...
table_tests[2,] <- paste0(round(table_tests[2,],2),"") #placeholder
table_tests[3,] <- paste0(round(as.numeric(table_tests[3,]),2),r3)
table_tests[4,] <- paste0(round(as.numeric(table_tests[4,]),2),r4)
table_tests[5,] <- paste0(round(as.numeric(table_tests[5,]),2),r5)
table_tests[6,] <- paste0(round(as.numeric(table_tests[6,]),2),r6)
table_tests[8,] <- paste0(round(as.numeric(table_tests[8,]),2),"")
table_tests[9,] <- paste0(round(as.numeric(table_tests[9,]),2),r9)
table_tests[10,] <- paste0(round(as.numeric(table_tests[10,]),2),r10)
table_tests[11,] <- paste0(round(as.numeric(table_tests[11,]),2),r11)
table_tests[12,] <- paste0(round(as.numeric(table_tests[12,]),2),r12)
table_tests[14,] <- paste0(round(as.numeric(table_tests[14,]),2),"")
table_tests[15,] <- paste0(round(as.numeric(table_tests[15,]),2),r15)
table_tests[16,] <- paste0(round(as.numeric(table_tests[16,]),2),r16)
table_tests[17,] <- paste0(round(as.numeric(table_tests[17,]),2),r17)
table_tests[18,] <- paste0(round(as.numeric(table_tests[18,]),2),r18)
table_tests[20,] <- paste0(round(as.numeric(table_tests[20,]),2),"")
table_tests[21,] <- paste0(round(as.numeric(table_tests[21,]),2),r21)
table_tests[22,] <- paste0(round(as.numeric(table_tests[22,]),2),r22)
table_tests[23,] <- paste0(round(as.numeric(table_tests[23,]),2),r23)
table_tests[24,] <- paste0(round(as.numeric(table_tests[24,]),2),r24)
table_tests[26,] <- paste0(round(as.numeric(table_tests[26,]),2),"")
table_tests[27,] <- paste0(round(as.numeric(table_tests[27,]),2),r27)
table_tests[28,] <- paste0(round(as.numeric(table_tests[28,]),2),r28)
table_tests[29,] <- paste0(round(as.numeric(table_tests[29,]),2),r29)
table_tests[30,] <- paste0(round(as.numeric(table_tests[30,]),2),r30)
...
df <- as.matrix(df)
rownames(df) <- c("AE VaR","AE ES", "UC","CC","DQ","AE VaR","AE CVaR","UC","CC","DQ","AE VaR","AE CVaR","UC","CC","DQ","AE VaR","AE CVaR","UC","CC","DQ","AE VaR","AE CVaR","UC","CC","DQ")
```

<!--# description @stephane -->
\noindent The description of table \@ref(tab:Table4) is still a work in progress, but the table basically shows the actual over expected exceedance hit for the VaR, the CVaR. The unconditional coverage test by @kupiec1995, the conditional coverage by @christoffersen2001 and the dynamic quantile test of @engle2004. Interpretation will follow and ofcourse formatting etc..

\newpage

```{r table4, echo=F, results='asis'}
df %>% kbl(booktabs=T, format="latex",caption = "VaR and CVaR tests",
  ...
  footnote(general = "Table contains the ... ratio, ... ratio, the ... test statistic, ... test statistic and the ... test statistic. *, ** and *** point out significance at 10%, 5% and 1% level.",threeparttable = T,footnote_as_chunk = F, escape=F, general_title = "Notes") 
```
...
V2Table_long <- V2Table %>%
  tidyr::gather("Stat", "Value", -c(dist, model))
...
A first test performed is the Ljung-box test on the absolute value of the standardized residuals of the GARCH models.
```{r echo=FALSE, results='asis'}
#LJUNG-BOX TEST ON STANDARDIZED ABSOLUTE RESIDUALS
...
Lj.box.test.function <- function(garchmodel = garchfit.sGARCH, n.list = 1, n.lag = 22){
  tmp.1 <- Box.test(abs(rugarch::residuals(garchmodel[[n.list]], standardize = T)),n.lag, "Ljung-Box") 
...
LJ.test.df %>% kbl(caption = "Ljung Box Test on standardized residuals in absolute values",
      label = 'LjboxTable',
      booktabs = T,
      position = "h!",
      digits = 3 )%>%
  kable_classic(full_width = F)%>%
  footnote(general_title = "Notes",general=c(paste0("Table displays the Ljung box statistics value for the absolute values of the residuals of the models analyzed. The underlying data is the daily return series of the Euro Stoxx 50 for the period between ", format(min(index(R)))," and ",format(max(index(R))),"."), "*, ** and *** point out respectively significance at 10%, 5% and 1% level.", "The null hypothesis is described as follows:     $H_0$: $Corr$(|$Z_t$|,|$Z_{t-1}$|)=$Corr$(|$Z_t$|,|$Z_{t-2}$|)= $...$ =$Corr$(|$Z_t$|,|$Z_{t-22}$|) = $0$"),threeparttable = T,footnote_as_chunk = F, escape=F) %>%
  kable_styling(latex_options = "scale_down") %>% landscape()
```
...
LJ.Test.2 <- vector(mode = "list", length = length(Models.garch))
names(LJ.Test.2) <- Models.garch
for(i in 1:length(LJ.Test.2)){
  LJ.Test.2[[i]] <- matrix(NA, nrow = 2, ncol = length(distributions), dimnames = list(c("Statistic", "P Values"), names(Table.3)))
}
...
for(i in 1:length(distributions)){
  LJ.Test.2[[1]][,i] <- Lj.box.test.function.2(garchmodel = garchfit.sGARCH, n.list = i)
    LJ.Test.2[[2]][,i] <- Lj.box.test.function.2(garchmodel = garchfit.eGARCH, n.list = i)
      LJ.Test.2[[3]][,i] <- Lj.box.test.function.2(garchmodel = garchfit.fGARCH.AVGARCH, n.list = i)
        LJ.Test.2[[4]][,i] <- Lj.box.test.function.2(garchmodel = garchfit.fGARCH.NAGARCH, n.list = i)
          LJ.Test.2[[5]][,i] <- Lj.box.test.function.2(garchmodel = garchfit.gjrGARCH, n.list = i)
            LJ.Test.2[[6]][,i] <- Lj.box.test.function.2(garchmodel = garchfit.fGARCH.TGARCH, n.list = i)
              LJ.Test.2[[7]][,i] <- Lj.box.test.function.2(garchmodel = garchfit.iGARCH, n.list = i)
                LJ.Test.2[[8]][,i] <- Lj.box.test.function.2(garchmodel = garchfit.EWMA, n.list = i)
}

LJ.Test.2 <- lapply(LJ.Test.2, round, 3)

for(i in 1:length(LJ.Test.2)){
LJ.Test.2[[i]][1,which(LJ.Test.2[[i]][2,]>=0.10)] <- LJ.Test.2[[i]][1,which(LJ.Test.2[[i]][2,]>=0.10)]
LJ.Test.2[[i]][1,which(LJ.Test.2[[i]][2,]<0.10&LJ.Test.2[[i]][2,]>0.05)] <- paste0(LJ.Test.2[[i]][1,which(LJ.Test.2[[i]][2,]<0.10&LJ.Test.2[[i]][2,]>0.05)],"*")
LJ.Test.2[[i]][1,which(LJ.Test.2[[i]][2,]<=0.05&LJ.Test.2[[i]][2,]>0.01)] <- paste0(LJ.Test.2[[i]][1,which(LJ.Test.2[[i]][2,]<=0.05&LJ.Test.2[[i]][2,]>0.01)],"**")
LJ.Test.2[[i]][1,which(LJ.Test.2[[i]][2,]<=0.01)] <- paste0(LJ.Test.2[[i]][1,which(LJ.Test.2[[i]][2,]<=0.01)],"***")
LJ.Test.2[[i]] <- LJ.Test.2[[i]][-2,]
}

LJ.test.matrix.2 <- cbind(LJ.Test.2[[1]],LJ.Test.2[[2]],LJ.Test.2[[3]],LJ.Test.2[[4]],LJ.Test.2[[5]],LJ.Test.2[[6]],LJ.Test.2[[7]],LJ.Test.2[[8]])
colnames(LJ.test.matrix.2) <- Models.garch.clean
LJ.test.df.2 <- as.data.frame(LJ.test.matrix.2)

LJ.test.df.2 %>% kbl(caption = "Ljung Box Test on standardized squared residuals",
      label = 'LjboxTablesquared',
      booktabs = T,
      position = "h!",
      digits = 3 )%>%
  kable_classic(full_width = F)%>%
  footnote(general_title = "Notes",general=c(paste0("Table displays the Ljung box statistics value for the squared residuals of the models analyzed. The underlying data is the daily return series of the Euro Stoxx 50 for the period between ", format(min(index(R)))," and ",format(max(index(R))),"."), "*, ** and *** point out respectively significance at 10%, 5% and 1% level.", "The null hypothesis is described as follows:     $H_0$: $Corr$($Z_t^2$,$Z_{t-1}^2$)=$Corr$($Z_t^2$,$Z_{t-2}^2$)= $...$ =$Corr$($Z_t^2$,$Z_{t-22}^2$) = $0$"),threeparttable = T,footnote_as_chunk = F, escape=F) %>%
  kable_styling(latex_options = "scale_down") %>% landscape()
```
...
archlmtest = function (x, lags = 22, demean = FALSE, n.list = 1){
...
Other diagnostics? robustness analysis?
...
```{r aic1, fig.align='center',fig.cap='Goodness of fit symmetric GARCH and distributions', out.width='90%',fig.pos='h', echo=F}
...
```{r aic2, fig.align='center',fig.cap='Goodness of fit symmetric GARCH and other distributions', out.width='90%',fig.pos='h', echo=F}
...
```{r aic3, fig.align='center',fig.cap='Goodness of fit asymmetric GARCH and symmetric distributions', out.width='90%',fig.pos='h', echo=F}
...
```{r aic4, fig.align='center',fig.cap='Goodness of fit asymmetric GARCH and symmetric distributions', out.width='90%',fig.pos='h', echo=F}
...
[^appendices-5]: Although we have to note that for some models like TGARCH and AVGARCH with SGED distribution the AIC was double of other models and therefore these models seem to work very poorly or are mispecified.
