...
[note for prof. Annaert: choices: n.start = 1500 days before the end of the series, refit.every = 252 (trading days in a year), solver = hybrid using a cluster = 10 to run on 10 cores to speed up the process of estimation of the roll object (took 5-10 minutes per backtest with some solvers, now with parallel package...)]{.correction}.
```
...
gettests(VaR.sstd.egarch)
report(egarch.sstdback, type = "VaR") #if we use the rugarch method we get the same LRuc and LRcc, only thing that is additional is the amount of exceed and actual exceed that is present in the report and not in the BacktestVaR

gettests(VaR.sstd.egarch)
gettests(VaR.norm.egarch)

```

```{r tableStephane, echo=F, results='asis'}
## table %>% kbl() %>%... 
...
table_tests <- matrix(nrow=25,ncol=5)
rownames(table_tests) <- c("SGED","AP-ratio", "UC","CC","DQ","GED","AP-ratio","UC","CC","DQ","ST","AP-ratio","UC","CC","DQ","T","AP-ratio","UC","CC","DQ","Norm","AP-ratio","UC","CC","DQ")
...
table_tests[7,]<- c(e_ged$ae, gjr_ged$ae,t_ged$ae,na_ged$ae,av_ged$ae)
table_tests[12,]<- c(e_sstd$ae, gjr_sstd$ae,t_sstd$ae,na_sstd$ae,av_sstd$ae)
table_tests[17,]<- c(e_std$ae, gjr_std$ae,t_std$ae,na_std$ae,av_std$ae)
table_tests[22,]<- c(e_norm$ae, gjr_norm$ae,t_norm$ae,na_norm$ae,av_norm$ae)

table_tests[3,] <- c(e_sged$uc[1], gjr_sged$uc[1],t_sged$uc[1],na_sged$uc[1],av_sged$uc[1])
table_tests[8,]<- c(e_ged$uc[1], gjr_ged$uc[1],t_ged$uc[1],na_ged$uc[1],av_ged$uc[1])
table_tests[13,]<- c(e_sstd$uc[1], gjr_sstd$uc[1],t_sstd$uc[1],na_sstd$uc[1],av_sstd$uc[1])
table_tests[18,]<- c(e_std$uc[1], gjr_std$uc[1],t_std$uc[1],na_std$uc[1],av_std$uc[1])
table_tests[23,]<- c(e_norm$uc[1], gjr_norm$uc[1],t_norm$uc[1],na_norm$uc[1],av_norm$uc[1])

table_tests[4,] <- c(e_sged$cc[1], gjr_sged$cc[1],t_sged$cc[1],na_sged$cc[1],av_sged$cc[1])
table_tests[9,]<- c(e_ged$cc[1], gjr_ged$cc[1],t_ged$cc[1],na_ged$cc[1],av_ged$cc[1])
table_tests[14,]<- c(e_sstd$cc[1], gjr_sstd$cc[1],t_sstd$cc[1],na_sstd$cc[1],av_sstd$cc[1])
table_tests[19,]<- c(e_std$cc[1], gjr_std$cc[1],t_std$cc[1],na_std$cc[1],av_std$cc[1])
table_tests[24,]<- c(e_norm$cc[1], gjr_norm$cc[1],t_norm$cc[1],na_norm$cc[1],av_norm$cc[1])

table_tests[5,] <- c(e_sged$dq$stat, gjr_sged$dq$stat,t_sged$dq$stat,na_sged$dq$stat,av_sged$dq$stat)
table_tests[10,]<- c(e_ged$dq$stat, gjr_ged$dq$stat,t_ged$dq$stat,na_ged$dq$stat,av_ged$dq$stat)
table_tests[15,]<- c(e_sstd$dq$stat, gjr_sstd$dq$stat,t_sstd$dq$stat,na_sstd$dq$stat,av_sstd$dq$stat)
table_tests[20,]<- c(e_std$dq$stat, gjr_std$dq$stat,t_std$dq$stat,na_std$dq$stat,av_std$dq$stat)
table_tests[25,]<- c(e_norm$dq$stat, gjr_norm$dq$stat,t_norm$dq$stat,na_norm$dq$stat,av_norm$dq$stat)

#PVALUES LOOPS

#KABLE OUTPUT
df <- table_tests[-c(1,6,11,16,21),] %>% as.data.frame()
colnames(df) <- c("EGARCH", "GJRGARCH", "TGARCH", "NAGARCH", "AVGARCH")
row_group_label_fonts <- list(
list(bold = T, italic = T)
)
df %>% kbl(booktabs=T, format="latex") %>% 
  kable_styling() %>%
  pack_rows("Panel A: SGED", 1,4) %>%
  pack_rows("Panel B: GED", 5,8) %>%
  pack_rows("Panel C: ST", 9,12) %>%
  pack_rows("Panel D: T", 13,17) %>%
  pack_rows("Panel E: N", 17,20) %>% add_indent(c(1:16, 18:20))
```
...
```{r aic1, fig.align='center',fig.cap='Goodness of fit symmetric GARCH and distributions', out.width='70%',fig.pos='h', echo=F}
...
```{r aic2, fig.align='center',fig.cap='Goodness of fit symmetric GARCH and other distributions', out.width='70%',fig.pos='h', echo=F}
...
```{r aic3, fig.align='center',fig.cap='Goodness of fit asymmetric GARCH and symmetric distributions', out.width='70%',fig.pos='h', echo=F}
...
```{r aic4, fig.align='center',fig.cap='Goodness of fit asymmetric GARCH and symmetric distributions', out.width='70%',fig.pos='h', echo=F}
...
[^appendices-5]: Although we have to note that for some models like TGARCH and AVGARCH with SGED distribution the the AIC was double of other models and therefore these models seem to work very poorly or are mispecified.
